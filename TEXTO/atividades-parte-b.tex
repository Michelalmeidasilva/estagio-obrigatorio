\section{Migração de Arquitetura Bridge para JS (Período: 16/10 a 31/10)}

A mudança exigiu a refatoração de uma biblioteca interna, com modificações diretas no código nativo (Android/iOS) para adequação à Nova Arquitetura (JSI/Turbo Modules).

O principal desafio metodológico foi o ciclo de feedback (build/test cycle) do projeto principal, que consumia aproximadamente 40 minutos por alteração. Essa sobrecarga tornava a depuração Iterativa impraticável e arriscada.

Para mitigar esse risco e aumentar a agilidade, foi adotada a estratégia de desenvolvimento em ambiente isolado (Sandboxing). A correção foi implementada e validada em um aplicativo de demonstração, permitindo ciclos rápidos de desenvolvimento e depuração antes da integração.
...
A implementação utiliza a arquitetura legada, empregando o módulo `NativeModules`. Na camada JS da aplicação, a comunicação com o módulo nativo é estabelecida por meio da importação de `NativeModules` do pacote `react-native`. O código completo desta implementação pode ser consultado na Listagem \ref{lst:legacy-js} do Apêndice \ref{apendice:migracao}.

No Código Nativo (Android - Java), foi realizada a implementação de um método chamado `getHelloWorld` para o retorno de uma string simples e um método `getName` para o retorno do nome do módulo nativo, conforme detalhado na Listagem \ref{lst:legacy-java} do Apêndice \ref{apendice:migracao}.

Para a plataforma iOS (Objective-C), de forma análoga ao código em Java, a implementação foi realizada conforme detalhado na Listagem \ref{lst:legacy-objc} do Apêndice \ref{apendice:migracao}.


\subsubsection{Funcionamento da Bridge:}
\begin{itemize}
    \item \textbf{Chamada JS:} O JS chamava HelloWorldModule.getHelloWorld()
    ...
    \item \textbf{Comunicação:} A mensagem JSON era enviada da thread JS para a thread Nativa, atravessando a Bridge
    ...
    \item \textbf{Retorno:} O resultado era serializado e retornado via callback/promise para o JS
\end{itemize}
...
\textbf{Definição da Interface de API (Spec - TypeScript)}: Um arquivo de especificação define o contrato da API de forma explícita na camada JS, conforme apresentado na Listagem \ref{lst:new-arch-spec} do Apêndice \ref{apendice:migracao}.


\textbf{Implementação Nativa (Android - Kotlin)}

Na implementação nativa, foi utilizada uma interface gerada que permitiu o acesso direto ao runtime JS, como pode ser visto na Listagem \ref{lst:new-arch-java} do Apêndice \ref{apendice:migracao}.


\textbf{Código em JS}

A interface de uso no JS conservou estrutura similar, mas passou a acessar diretamente o módulo C++ por meio da JSI (ver Listagem \ref{lst:new-arch-js} no Apêndice \ref{apendice:migracao}).


\textbf{Implementação Nativa (iOS - Objective-C)}

Para a plataforma iOS, implementou-se o código nativo em Objective-C, conforme demonstrado na Listagem \ref{lst:new-arch-objc} do Apêndice \ref{apendice:migracao}.
...
\textbf{Descrição:} Um aplicativo React Native mínimo foi criado e configurado com os mesmos parâmetros da aplicação principal. O objetivo deste ambiente foi isolar totalmente a biblioteca do restante do ecossistema complexo do projeto principal.
...
\begin{itemize}
    ...
    \item A execução da funcionalidade central, confirmando que a comunicação entre a camada JS e as novas implementações nativas estava ocorrendo conforme a especificação da API.
\end{itemize}
...
\subsection{Testes em ambientes pré-produtivos}
Esta etapa representou o ``ensaio geral'' antes da produção.

\textbf{Descrição:} O ambiente de Pré-Produção é um espelho fiel do ambiente de Produção, utilizando a mesma infraestrutura e, idealmente, uma réplica recente (e anonimizada) do banco de dados produtivo.

\textbf{Execução:} O build aprovado em Homologação foi promovido para este ambiente.

\textbf{Validação:} Aqui ocorreu a validação final de aceitação por parte dos stakeholders e Product Owners. Além dos testes funcionais, foram avaliados aspectos não-funcionais, como a performance da comunicação JSI sob carga e a interação com serviços de backend reais. Esta foi a última verificação de segurança antes da exposição aos usuários finais.
...
O módulo nativo, empacotado originalmente com o react-native-builder-bob, tornou-se obsoleto e causava erro de renderização dos componentes de layout após o upgrade do React Native. Utilizando o boilerplate e as ferramentas de linking do react-native-builder-bob, foram reconfigurados os wrappers nativos. Este processo exigiu a criação de um arquivo de especificação TypeScript (.d.ts), que define formalmente a interface do módulo, garantindo a tipagem estática. Em seguida, foi adaptado os entry points nativos (em Kotlin para Android e Swift para iOS) para implementar a interface gerada pelo JSI, o que assegurou a comunicação síncrona, tipada e de alta performance entre o JS e o código nativo. O upgrade do React Native da versão 0.71.5 para a 0.81.4 envolveu diversas mudanças estruturais, de dependências e de configuração. Na \autoref{tab:mudancas-rn-versao} está um resumo das principais alterações realizadas, conforme o relatório do Upgrade Helper.

\begin{table}[h!]
    \centering
    % 1. O caption vem ANTES do label
    \caption{Mudanças de Versão e Dependências}
    % 2. O label deve ser igual ao usado no \autoref (com o prefixo tab:)
    \label{tab:mudancas-rn-versao}
...
\begin{itemize}
    \item Inclusão dos pacotes Interface de Linha de Comando (CLI) da comunidade na versão 20.0.0 (@react-native-community/cli, cli-platform-android, cli-platform-ios).
...
\subsection{Execução do módulo em um projeto de demonstração}
Devido às dificuldades no processo de build do projeto principal, a validação da crítica mudança na dependência nativa foi realizada em um aplicativo de demonstração. Esta abordagem foi adotada para agilizar a depuração, contornando o build lento do projeto principal. No ambiente isolado do sample app, foi possível:
\begin{itemize}
...
\subsection{Testes em ambiente de homologação }
A nova versão da biblioteca foi atualizada no aplicativo principal e um build foi gerado para o ambiente de homologação. A equipe de qualidade executou um plano de testes de regressão focado:
\begin{itemize}
