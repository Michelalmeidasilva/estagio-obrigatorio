...
A Figura \ref{fig:com_async} ilustra o processo de criação de um componente e as funções que possibilitam a comunicação entre o código React Native e os módulos nativos.
...
\begin{figure}[h!]
    ...
    \label{fig:com_async}
    \includegraphics[width=\textwidth]{IMAGENS/comunicao-async-rn.png} % Placeholder para a imagem
    \vspace{0.3em}
...
\textbf{Definição da API (Spec - TypeScript)}: Um arquivo de especificação define o contrato da API de forma explícita na camada javascript, da seguinte maneira:
