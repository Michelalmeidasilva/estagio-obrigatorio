...
\begin{description}
    ...
    \item[Metodologia Ágil:] É uma abordagem iterativa para o gerenciamento de projetos e desenvolvimento de software que ajuda as equipes a entregar valor aos seus clientes de forma mais rápida e com menos dores de cabeça.
...
\begin{description}
    \item[Arquitetura Monolítica:] Modelo tradicional onde a aplicação é uma única unidade coesa. Todos os componentes (UI, lógica, dados) são fortemente acoplados.
    \item[Arquitetura de Microsserviços:] Padrão onde uma aplicação complexa é composta por serviços pequenos e independentes, cada um com uma capacidade de negócio específica, comunicando-se via APIs \cite{microservices}.
...
\begin{description}
    \item[Node.js:] Ambiente de execução JavaScript assíncrono construído sobre o motor V8. Seu modelo de I/O não bloqueante o torna ideal para microsserviços e APIs de alta performance \cite{nodejs}.
    \item[React:] Biblioteca JavaScript para construção de interfaces de usuário baseada em componentes e declarativa. Utiliza o Virtual DOM para atualizações eficientes \cite{react}. Conceitos chave incluem Componentes, Reatividade, Gerenciamento de Estados e Hooks.
    \item[Next.js:] Framework React para produção que oferece renderização híbrida (SSR e SSG) e sistema de roteamento baseado em arquivos \cite{nextjs}.
\end{description}
...
\begin{description}
    ...
    \item[React Native:] Framework para desenvolvimento multiplataforma usando JavaScript e React \cite{reactnative}.
    \begin{itemize}
        \item \textbf{Arquitetura Legada (Bridge):} Comunicação assíncrona e serializada (JSON) entre JavaScript e Nativo.
        \item \textbf{Nova Arquitetura (JSI/Fabric/TurboModules):} Permite comunicação síncrona e direta (C++) entre JavaScript e Nativo, eliminando a "ponte" e melhorando a performance.
        \item \textbf{Ferramentas:} \textit{React-native-builder-bob} para criação de bibliotecas e \textit{Upgrade Helper} para migração de versões.
    \end{itemize}
\end{description}
...
\begin{description}
    \item[Design System:] Fonte única de verdade para design e desenvolvimento. O "Pulso" (RD Saúde) provê tokens, componentes e documentação para garantir consistência.
    ...
    \item[Lighthouse:] Ferramenta de auditoria automatizada que avalia Performance, Acessibilidade, Boas Práticas, SEO e PWA.
    \item[Web Vitals:] Iniciativa do Google para métricas de qualidade de experiência do usuário.
    \begin{itemize}
        \item \textbf{Core Web Vitals:} LCP (carregamento), CLS (estabilidade visual) e INP (interatividade).
    \end{itemize}
\end{description}
...
\begin{description}
    \item[Agentes Conversacionais:] Evolução do Dialogflow CX, integrando fluxos determinísticos (baseados em estados) e generativos (baseados em LLMs e Playbooks).
    \item[RAG (Retrieval-Augmented Generation):] Técnica que otimiza respostas de LLMs usando bases de conhecimento confiáveis (Data Stores).
