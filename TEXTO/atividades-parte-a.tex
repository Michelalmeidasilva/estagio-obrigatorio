...
A simples criação de endpoints individuais em cada microsserviço levaria a um acoplamento forte com o front-end e a uma complexidade de gerenciamento. Aplicando os princípios de Design de Software, foi implementada uma Interface de Programação de Aplicações (API) Gateway para desacoplamento entre regras de negócios da aplicação.
...
\begin{itemize}
    \item \textbf{Manipulação Direta do Document Object Model (DOM) para UI:} A atualização visual da barra de progresso, que precisa ser fluida, foi desacoplada do estado do React. Foi utilizado o hook useRef para obter uma referência direta ao elemento da barra e atualizar seu estilo (transform: scaleX(...)) diretamente, contornando o ciclo de renderização do React para essa animação específica.
    \item \textbf{Throttling para o Estado React:} O estado global do React (ex: o texto "01:32 / 05:00") ainda precisava ser atualizado, mas não com a mesma frequência. Foi implementada uma função de throttle (usando a biblioteca lodash, por exemplo) para garantir que o setState fosse chamado no máximo uma vez a cada 250ms, garantindo a atualização da informação sem sobrecarregar a aplicação.
...
A integração dentro das telas que fazem uso do componente foi finalizada. Esta fase envolveu a passagem das props necessárias (como a URL do vídeo) e a conexão de callbacks (como onEnded) com a lógica de negócio da página que o contém.
...
A complexidade de uma atualização do React Native está no código JavaScript, e também no ecossistema nativo subjacente. É uma tarefa de configuração dos componentes nativos, atualização de bibliotecas do React-Native e correção de builds.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Análise de Breaking Changes:} Antes de iniciar, foi utilizada a ferramenta oficial react-native-upgrade-helper. Ela fornece um diff detalhado entre as versões, mostrando todas as alterações necessárias em arquivos de template nativos e de configuração, o que foi crucial para estimar o esforço e os pontos de risco.
