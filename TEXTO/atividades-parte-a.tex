...
\begin{itemize}
    \item \textbf{Manipulação Direta do DOM para UI:} A atualização visual da barra de progresso, que precisa ser fluida, foi desacoplada do estado do React. Foi utilizado o hook useRef para obter uma referência direta ao elemento da barra e atualizar seu estilo (transform: scaleX(...)) diretamente, bypassando o ciclo de renderização do React para essa animação específica.
...
\subsection{Tarefa: Otimização de Performance e Métricas de Core Web Vitals (CWV).}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Para resolver o CLS, foram especificadas as dimensões exatas de cada imagem, para que o navegador pudesse reservar o espaço em tela antes do download ser concluído, evitando o "salto" no layout.
    \end{itemize}
    \item \textbf{Validação:} Após as otimizações, foram executadas novamente as ferramentas de profiling para validar que as métricas de CWV estavam dentro dos limites recomendados, tratando a performance como um requisito funcional do projeto.
