...
\subsection{Tarefa: Design e Implementação do API Gateway e do Contrato de Comunicação entre microsserviços.}

A simples criação de endpoints individuais em cada microsserviço levaria a um acoplamento forte com o front-end e a uma complexidade de gerenciamento. Aplicando os princípios de Design de Software, foi implementada uma API para desacoplamento entre regras de negócios da aplicação.

\subsection{Execução Baseada em Engenharia de Software}
\begin{itemize}
    \item \textbf{Análise de Requisitos:} Em colaboração com a equipe de front-end, foram mapeados os casos de uso das telas. Isso definiu os dados que o Gateway precisaria agregar.
    \item \textbf{Design da Arquitetura:} Foi desenhado o Gateway como um serviço Node.js simples, cuja única responsabilidade (Princípio 'S' do SOLID) era receber requisições do cliente, orquestrar chamadas para os microsserviços internos e agregar as respostas. Isso desacoplou os clientes da topologia interna da rede de serviços.
    \item \textbf{Construção e Qualidade:} Foi definido um contrato de comunicação claro usando a especificação OpenAPI (Swagger). Isso permitiu que as equipes de front-end e back-end trabalhassem em paralelo, usando o contrato documentado como fonte da verdade. O Gateway também centralizou a lógica de autenticação de rotas e o tratamento de CORS, simplificando os serviços internos. A implementação seguiu padrões de código limpo para garantir a manutenibilidade futura.
\end{itemize}

\section{Implementação de Componente de Vídeo (Período: 06/10 a 25/11)}

\subsection{Tarefa: Design, Implementação e Otimização de um Player de Vídeo Reutilizável.}
...
Seguindo o princípio de Separação de Responsabilidades e a filosofia da Arquitetura Limpa, a lógica de negócio foi completamente isolada da camada de apresentação.

\begin{itemize}
    \item \textbf{Custom Hook (useVideoPlayerState):} Foi criado um custom hook que serve como o "cérebro" do componente. Ele encapsula toda a interação com a API da biblioteca de vídeo (ex: React Player) e gerencia a FSM interna do player.
    \item \textbf{Estados da FSM:} O hook gerencia os seguintes estados: IDLE, LOADING, PLAYING, PAUSED, SEEKING, ENDED, ERROR.
    \item \textbf{API Exposta pelo Hook:} O hook retorna um objeto bem definido para o componente de interface de usuário (UI), contendo:
    \begin{itemize}
...
\begin{itemize}
    \item \textbf{Manipulação Direta da DOM para UI:} A atualização visual da barra de progresso, que precisa ser fluida, foi desacoplada do estado do React. Foi utilizado o hook useRef para obter uma referência direta ao elemento da barra e atualizar seu estilo diretamente, contornando o ciclo de renderização do React para essa animação específica.
    \item \textbf{Throttling para o Estado React:} O estado global do React ainda precisava ser atualizado, mas não com a mesma frequência. Foi implementada uma função de throttle para garantir que o setState fosse chamado no máximo uma vez a cada 250ms, garantindo a atualização da informação sem sobrecarregar a aplicação.
    \item \textbf{Memoização:} Todos os sub-componentes da UI (ex: <PlayPauseButton />, <VolumeControl />) foram envolvidos em React.memo, e as funções de callback passadas como propriedades foram estabilizadas com useCallback. Isso previne re-renderizações desnecessárias quando o estado pai muda, mas as propriedades específicas daquele componente filho permanecem as mesmas.
\end{itemize}
...
A integração dentro das telas que fazem uso do componente foi finalizada. Esta fase envolveu a passagem das propriedades necessárias (como a URL do vídeo) e a conexão de callbacks (como onEnded) com a lógica de negócio da página que o contém.

\section{Estudos de Viabilidade Técnica para Funcionalidades de Vídeos com interatividade}

\subsection{Tarefa: Análise de Trade-offs Arquiteturais para a Solução de vídeos interativos com hotspots.}

O estudo técnico envolveu uma análise de engenharia de sistemas, considerando custos, escalabilidade e manutenibilidade. Minha tarefa foi fornecer à liderança um relatório técnico para uma decisão informada.

\subsection{Execução}
\begin{itemize}
    \item \textbf{Análise de Requisitos Não-Funcionais:} Foram definidos os critérios de avaliação: latência, escalabilidade, custo por espectador/hora e complexidade de manutenção.
    \item \textbf{Relatório Técnico e Recomendação:} Nele, foi apresentada a análise comparativa e recomendada a abordagem, alinhando a solução técnica com os objetivos de negócio (velocidade de lançamento), com um plano para reavaliar a estratégia se a escala crescesse exponencialmente.
\end{itemize}

\section{Implementação de Lógicas para Descontos (Período: 04/10 a 06/11)}


\subsection{Tarefa: Garantia de Qualidade e Prevenção de Regressão em Código Legado Crítico.}
Foi modificado  um sistema legado e monolítico, sem testes e com alto impacto no negócio (cálculo de preços), sendo uma tarefa de risco, a prioridade absoluta foi garantir que o comportamento existente não fosse quebrado.

\subsection{Execução Baseada em Engenharia de Software}
\begin{itemize}
    \item \textbf{Engenharia Reversa e Testes de Caracterização:} Antes de alterar o código, foi aplicada a técnica de Testes de Caracterização. Foi criada uma suíte de testes que não validava o que o código deveria fazer, mas o que ele realmente fazia, incluindo seus comportamentos inesperados. Isso criou uma rede de segurança que "caracterizou" o comportamento do sistema.
    \item \textbf{Refatoração Baseada em Padrões:} Com a segurança dos testes, foi liderada a refatoração. O código original era uma violação do Princípio Aberto/Fechado (para adicionar um novo desconto, era preciso modificar a função existente). Foi implementado o Strategy Pattern, onde cada lógica de desconto se tornou uma classe separada com uma interface comum. A função principal agora apenas seleciona a "estratégia" correta, sem precisar conhecer seus detalhes.
...
Foram criados os testes necessários, realizada a validação da tarefa em conjunto com o time de qualidade e executado o deploy.

\section{Implementação de Funcionalidade para Imagens no Website (Período: 25/09 a 15/10)}

\subsection{Tarefa: Otimização de Performance e Métricas de CWV.}
...
\subsection{Execução Baseada em Engenharia de Software}
\begin{itemize}
    \item \textbf{Análise e Diagnóstico:} Foram utilizadas ferramentas de profiling (Lighthouse, Chrome DevTools) para medir o impacto da implementação inicial. O diagnóstico foi claro: o carregamento de múltiplas imagens de alta resolução estava atrasando o LCP e o carregamento tardio causava CLS.
    \item \textbf{Otimização Estrutural:} Apliquei uma abordagem multifacetada:
    \begin{itemize}
        \item Foi usado o componente <Image> do Next.js, que automaticamente serve imagens em formatos modernos (WebP) e tamanhos otimizados.
        \item Foi adicionada a prop priority na primeira imagem do carrossel. Isso é uma diretiva de engenharia que sinaliza ao navegador para priorizar o download deste recurso, melhorando diretamente o LCP.
        \item Para as demais imagens, foi implementado lazy loading, fazendo com que fossem carregadas apenas quando se aproximasse do viewport.
        \item Para resolver o CLS, foram especificadas as dimensões exatas de cada imagem, para que o navegador pudesse reservar o espaço em tela antes do download ser concluído, evitando a mudança no layout repentina.
    \end{itemize}
    \item \textbf{Validação:} Após as otimizações, foram executadas novamente as ferramentas de profiling para validar que as métricas de CWV estavam dentro dos limites recomendados, tratando a performance como um aspecto essencial do projeto.
...
\begin{table}[h!]
    ...
    \begin{tabular}{p{0.45\textwidth}|p{0.45\textwidth}}
        ...
        Validar a renderização condicional baseada em propriedade. & Renderizar o componente com a 'showArrows' como 'false' e verificar se os botões de navegação não existem no DOM. \\ \hline
    \end{tabular}
...
O projeto realizado em React Native estava com versões desatualizadas do framework, o que deixava expostos 
riscos de segurança, incompatibilidades com bibliotecas da comunidade, incompatibilidades com as versões mais recentes dos sistemas operacionais móveis (Android e iOS) e 
limitações de performance. Na tabela 3 é visto uma relação entre os pontos de alterações realizados, a descrição técnica da mudança e a solução 


\subsection{Tarefa: Gerenciamento de Dependências Nativas Conflitantes e do Processo de Build}

A complexidade de uma atualização do React Native está no código JS, e também no ecossistema nativo subjacente. É uma tarefa de configuração dos componentes nativos, atualização de bibliotecas do React-Native e correção de builds.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Análise de Breaking Changes:} Antes de iniciar, foi utilizada a ferramenta oficial react-native-upgrade-helper. Ela fornece a diferença dos códigos detalhado entre as versões, mostrando todas as alterações necessárias em arquivos de template nativos e de configuração, o que foi crucial para estimar o esforço e os pontos de risco.
        \item \textbf{Mapeamento de Dependências:} Foi criado um inventário de todas as bibliotecas de terceiros, verificando seus changelogs e issues no GitHub para confirmar a compatibilidade com a nova versão do React Native.
        \item \textbf{Plano de Execução:} Foi definido um plano em uma branch Git isolada (feature/upgrade-rn-0.xx), que incluía um plano de rollback claro: a qualquer momento, se um impedimento intransponível fosse encontrado, a branch seria descartada, garantindo zero impacto na main branch.
    \end{itemize}
...
\begin{table}[h!]
    ...
    \begin{tabular}{p{0.3\textwidth}|p{0.3\textwidth}|p{0.3\textwidth}}
        ...
        Smoke Test (Manual) & Build de Teste & A aplicação abre sem fechar inesperadamente após a instalação limpa em um dispositivo físico com Android 14 e iOS 17. \\ \hline
